...
module.exports = function(opts) {
  ...
  var filter = options.plugins && function(message) {
    if (typeof options.plugins === 'function') return options.plugins(message);
    return options.plugins.every(function(plugin) { return plugin[0] === '!'; })
      ? options.plugins.indexOf('!' + message.plugin) === -1
      : options.plugins.indexOf(message.plugin) !== -1;
  };

  return function(css, result) {
    var messagesToLog = filter ? result.messages.filter(filter) : result.messages;
